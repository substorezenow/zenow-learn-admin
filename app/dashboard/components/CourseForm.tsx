...
export default function CourseForm({
  ...
  useEffect(() => {
    if (course) {
      ...
      setFieldValue(
        "difficulty_level",
        (course.difficulty_level || "beginner").toLowerCase() as
          | "beginner"
          | "intermediate"
          | "advanced"
      );
      ...
      setFieldValue(
        "instructor_id",
        course.instructor_id && course.instructor_id !== "null"
      ...
      setFieldValue(
        "learning_outcomes",
        parseLearningOutcomes(course.learning_outcomes || null)
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (course) {
        ...
        if (formData.short_description !== course.short_description)
          updateData.short_description = formData.short_description || "";
        if (formData.banner_image !== course.banner_image)
          updateData.banner_image = formData.banner_image || "";
        if (formData.thumbnail_image !== course.thumbnail_image)
          updateData.thumbnail_image = formData.thumbnail_image || "";
        if (Number(formData.duration_hours) !== course.duration_hours)
          updateData.duration_hours = Number(formData.duration_hours) || 0;
        if (
          (formData.difficulty_level || "beginner").toLowerCase() !==
          course.difficulty_level
        )
          updateData.difficulty_level = (
            formData.difficulty_level || "beginner"
          ).toLowerCase() as "beginner" | "intermediate" | "advanced";
        if (Number(formData.price) !== course.price)
          updateData.price = Number(formData.price) || 0;
        if (formData.is_free !== course.is_free)
          updateData.is_free = formData.is_free ?? false;
        if (formData.is_published !== course.is_published)
          updateData.is_published = formData.is_published ?? false;
        if (formData.field_id !== course.field_id)
          updateData.field_id = formData.field_id || ""; // Keep as string
        if (formData.prerequisites !== course.prerequisites)
          updateData.prerequisites = formData.prerequisites || "";

        // Handle learning outcomes comparison
        const currentLearningOutcomes = parseLearningOutcomes(
          course.learning_outcomes || null
        );
        if (formData.learning_outcomes !== currentLearningOutcomes) {
          updateData.learning_outcomes = formatLearningOutcomes(
        ...
        const createData: CreateCourseRequest = {
          ...
          learning_outcomes: formatLearningOutcomes(
            formData.learning_outcomes || ""
          ),
